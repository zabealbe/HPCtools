...
def test_file_uploading(uploader):
    with patch("os.listdir", return_value=["file1.txt", "file2.txt"]), patch(
        ...
        with patch.object(uploader, "put") as mock_put, patch.object(
            uploader, "listdir_attr", return_value=[]
        ):
            uploader.upload(source, target)
            expected_calls = [
...
def test_directory_creation(uploader):
    with patch(
        "os.listdir", side_effect=lambda path: ["dir1"] if path == source else []
...
def test_rsync_call(rsync_uploader):
    ...
    with patch("os.system") as mock_os:
        rsync_uploader.upload(str(source_path), target, blacklist=["ignore.txt"])
        mock_os.assert_called_once_with(
            "rsync -avz --exclude=ignore.txt -e 'ssh -p 22 -i {0}' {1}/ {2}@{3}:{4}".format(
