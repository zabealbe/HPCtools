...
def mock_resource_filename(mocker):
    return mocker.patch('pkg_resources.resource_filename', return_value='/fake/path/job.bash.j2')
...
def test_ssh_runner(mock_resource_filename, mock_ssh_runner_run):
    ...
    runner = SSHRunner(
        workdir="/scratch/work/username/project",
         auth_kwargs={
            "hostname": "hostname",
            "username": "username",
    ...
    mock_resource_filename.assert_called_once_with('hpctools', 'templates/launch_scripts/job.bash.j2')
    mock_ssh_runner_run.assert_called_once_with(
        template='/fake/path/job.bash.j2',
        template_args={}
    )
